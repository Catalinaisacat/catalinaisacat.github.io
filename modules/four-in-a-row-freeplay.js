var jsPsychFourInARowFreePlay = (function (jspsych) {
    ...
    const info = {
        ...
        parameters: { 
            ...
            player: {
                // 1 if subject is playing black, 0 if playing white
                type: jspsych.ParameterType.INT,
                default: null
            }
        }
    ...
    class FourInARowFreePlay {
        constructor(jsPsych){
            ...
            this.current_state = [];
            // Keeps track of player's moves being made
            this.player_move = [];
            // Keeps track of opponent's moves being made
            this.ai_move = [];
        ...
        make_move(piece){
            ...
            if (!this.black_pieces.includes(piece) && !this.white_pieces.includes(piece)){
                // Records move
                this.current_state.push(piece);
                if (this.turn == this.player){
                    this.player_move.push(piece)
                    this.ai_move.push(-1)
                } else {
                    this.player_move.push(-1)
                    this.ai_move.push(piece)
                }
                if (this.turn == "Black"){
                    // this.mousedown_sound.play();
                    // setTimeout(()=>{this.mouseup_sound.play();}, 100)
        ...
        update_top(){
            if (this.game_result == "win"){
                ...
                if (this.tutorial){
                    this.top.innerHTML = `You won the game.`;
                }
                else{
                    let b =0.20;
                ...
                if (this.tutorial){
                    this.top.innerHTML = 'You lost the game.';
                }
                else{
                ...
                if (this.tutorial){
                    this.top.innerHTML = `You tied with the opponent`;
                }
                else{
                    this.top.innerHTML = `You tied and got $0.10 bonus reward!`;
                }
            } else if (this.free_play){
                this.top.innerHTML = `You are playing <u>${this.player}</u>`;//Game ${this.game_index}: ${this.player == this.turn ? "Your" : "Opponent's"}
            }
        }
        ...
        end_trial(){
            ...
            var trial_data = {
                ...
                player_move: this.player_move,
                ai_move: this.ai_move,
        ...
        trial(display_element, trial){
            ...
            <h2 id='top'>You are playing <u>${this.player}</u></br></br> Your turn to move.</h2>
            <div id="container">
